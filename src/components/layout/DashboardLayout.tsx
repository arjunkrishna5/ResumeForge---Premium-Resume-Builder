  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 font-sans overflow-hidden">
          ...
          <>
            ...
            >
              <div className="flex h-16 shrink-0 items-center justify-between px-6 border-b border-slate-200">
                ...
                <button onClick={() => setIsSidebarOpen(false)} className="p-1 -mr-2 text-slate-400 hover:text-navy cursor-pointer">
                  <X className="h-5 w-5" />
                </button>
              </div>
              
              <div className="flex-1 p-4 overflow-y-auto">
                <Link to="/builder" onClick={() => setIsSidebarOpen(false)} className="flex items-center justify-center gap-2 w-full rounded-lg bg-primary px-4 py-3 text-sm font-semibold text-white shadow-sm mb-6 cursor-pointer">
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          ...
                          isActive ? "bg-indigo-50 text-primary font-semibold" : "text-slate-600 hover:bg-slate-50 hover:text-navy"
                        )}
                      >
              ...
              <div className="shrink-0 p-4 border-t border-slate-200">
                <button onClick={() => { signOut(); setIsSidebarOpen(false); }} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-navy transition-colors w-full cursor-pointer">
      ...
      <main className="flex min-w-0 flex-1 flex-col relative h-full">
             ...
             <div onClick={() => navigate('/settings')} className="flex items-center gap-3 group cursor-pointer p-1 rounded-full hover:bg-slate-50 transition-colors">
               ...
               <div className="h-9 w-9 shrink-0 rounded-full bg-indigo-100 flex items-center justify-center overflow-hidden border border-slate-200">
                  {currentUser?.photoURL ? (
                    ...
                    <span className="text-primary font-bold text-sm">{getInitial(currentUser?.displayName, currentUser?.email)}</span>
                  )}
               </div>
             </div>
           </div>
        ...
        {/* Scrollable Content inside Main, or let the builder take over completely */}
        {/* We changed `overflow-hidden` on the main wrapper container to allow internal scrolling. */}
        <div className="flex-1 overflow-y-auto relative h-[calc(100vh-64px)]">
          <Outlet />
        </div>
      </main>
    </div>
